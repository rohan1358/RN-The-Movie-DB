...
export default class Stack extends Component {
  constructor() {
    ...
    this.state = {
      topPopulerMovie: [],
      topPopulerTv: [],
      urlTopMovie: `https://api.themoviedb.org/3/discover/movie?api_key=1fdb707d0fb3912e6be0909078fcb461&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page`,
  ...
  componentDidMount = () => {
    if (this.state.loading === true) {
      ...
      let video = null;
      let example = [];
      Axios.get(urlTopTv, '1').then((Response) => {
            ...
            <View
              ...
              <View>
                <Button
                  style={{marginTop: 10}}
                  transparent
                  onPress={() => this.setState({side: 'none'})}>
          ...
          <View style={{flex: 1, flexDirection: 'row', alignItems: 'center'}}>
            <View style={{flex: 1}}>
              <Button
                style={{marginLeft: 20}}
                transparent
                onPress={() => this.setState({side: 'flex'})}>
                <Icons name="menu-outline" size={30} color="#FF7314"></Icons>
              </Button>
            </View>
            <View
